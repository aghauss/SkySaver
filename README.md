...
Skysaver consists of an end-user application where users can enter their desired journey and find out whether there is a potential to save money, how high that potential is in % and to which country they should relocate their IP to get the highest savings. The tool is based on a dataset that was generated by running multiple webscrapers identical queries through different IP-Locations on a popular flight search service.

The repository contains all scripts to replicate the tool by rerunning the webscraper to generate sufficient dataset and retrain the model. The applications is based on an ExtraTreeRegressor Model predicting the potential saving in % and a ExtraTreeClassification Model predicting the country with the highest saving potential.
